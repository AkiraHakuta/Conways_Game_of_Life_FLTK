...
int LifeGame::arround_alive_num_twisted_Cylinder(int x1, int y1){
    int alive_count = 0;
    int x0 = (x1-1 + _x_size) % _x_size;                         
    int x2 = (x1+1) % _x_size;            // (x0,y0),(x1,y0),(x2,y0) 
    int y0 = (y1-1 + _y_size) % _y_size;  // (x0,y1),(x1,y1),(x2,y1)
...
int LifeGame::arround_alive_num_twisted_Torus(int x1, int y1){
    int alive_count = 0;
    int x0 = (x1-1 + _x_size) % _x_size;                         
    int x2 = (x1+1) % _x_size;            // (x0,y0),(x1,y0),(x2,y0) 
    int y0 = (y1-1 + _y_size) % _y_size;  // (x0,y1),(x1,y1),(x2,y1)
...
int LifeGame::arround_alive_num_closed_Rectangle(int x1, int y1){
    int alive_count = 0;
    int x0 = x1-1;                         
    int x2 = x1+1;  // (x0,y0),(x1,y0),(x2,y0) 
    int y0 = y1-1;  // (x0,y1),(x1,y1),(x2,y1)
    int y2 = y1+1;  // (x0,y2),(x1,y2),(x2,y2)
    if (x0 != -1 and y0 != -1 and (*curr_matrix)[x0][y0] == ALIVE)
        alive_count++;
    if (x0 != -1 and (*curr_matrix)[x0][y1] == ALIVE)
        alive_count++;
    if (x0 != -1 and y2 != _y_size and (*curr_matrix)[x0][y2] == ALIVE)
        alive_count++;
    if (y0 != -1 and (*curr_matrix)[x1][y0] == ALIVE)
        alive_count++;
    if (y2 != _y_size and (*curr_matrix)[x1][y2] == ALIVE)
        alive_count++;
    if (y0 != -1 and x2 != _x_size and (*curr_matrix)[x2][y0] == ALIVE)
        alive_count++;
    if (x2 != _x_size and (*curr_matrix)[x2][y1] == ALIVE)
        alive_count++;
    if (x2 != _x_size and y2 != _y_size and (*curr_matrix)[x2][y2] == ALIVE)
        alive_count++;
    return alive_count;
}
...
void LifeGame::rule(int x, int y, int mode){
    ...
    switch (mode){
        case TP_Torus:
            ...
            break;
        case TP_Klein_Bottle:
            alive_count = arround_alive_num_KleinBT(x, y);
            break;
        case TP_twisted_Cylinder:
            alive_count = arround_alive_num_twisted_Cylinder(x, y);
            break;
        case TP_twisted_Torus:
            alive_count = arround_alive_num_twisted_Torus(x, y);
            break;
        case TP_closed_Rectangle:
            alive_count = arround_alive_num_closed_Rectangle(x, y);
            break;
    }
...
int main() {
    ...
    lgame.reset_game();
    lgame.set_curr_matrix(&list);
    cout << "TP_Klein_Bottle" << endl;
    lgame.curr_matrix_print();  
    for (int i = 0; i < 20; i++){
        lgame.next_gen(TP_Klein_Bottle);
        lgame.curr_matrix_print();  
    }
    ...
    lgame.reset_game();
    lgame.set_curr_matrix(&list);
    cout << "TP_closed_Rectangle" << endl;
    lgame.curr_matrix_print();  
    for (int i = 0; i < 20; i++){
        lgame.next_gen(TP_closed_Rectangle);
        lgame.curr_matrix_print();  
    }
